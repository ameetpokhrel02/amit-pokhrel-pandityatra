...
class Command(BaseCommand):
    help = 'Check for video call timeouts (5 mins join window) and handle missed calls'
    ...
    def handle(self, *args, **options):
        ...
        for room in active_rooms:
            ...
            # If we are at least 5 minutes past the scheduled start
            if now > (start_dt + timedelta(minutes=5)):
                ...
                if count == 1:
                    ...
                    # Check if they have been waiting for at least 5 minutes
                    if now > (participant.joined_at + timedelta(minutes=5)):
                        # Journey Rule: System auto-ends if partner didn't join within 5 mins of waiting
                        room.status = 'ended'
                        room.ended_at = now
                        room.save()
                        ...
                        self.stdout.write(self.style.SUCCESS(f"Auto-ended room {room.room_name} due to {missing_role} missing."))
                    ...
                    room.ended_at = now
                    room.save()
                    self.stdout.write(self.style.WARNING(f"Auto-ended room {room.room_name} - neither party joined."))
